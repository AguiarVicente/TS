...
class Mapeando<Chave, Valor>{
  itens: Array<Chave, Valor> = new Array<Chave, Valor>()

  obter(chave: Chave){
    const resultado = this.itens.filter(
      (item => item.chave === chave)
      )
      return resultado ? resultado[0] : null
  }

  colocar(par: {chave: Chave, valor: Valor}){
  ...
  limpar(){
    this.itens = new Array<Chave, Valor>()
  }
...
const mapear = new Mapeando<number, string>()
